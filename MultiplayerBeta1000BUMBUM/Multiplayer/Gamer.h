...
namespace MultiplayerLowClass
{
	...
	public:
		...
		Player():CURRENTWALKINGSTAT(MultiplayerEnumerations::PSY_DOWN){this->p_s_Position = new MultiplayerStructs::Vector2D;p_s_Velocity = new MultiplayerStructs::Vector2D; p_s_Position->X = 0;p_s_Position->Y = 0;}
		Player(int x, int y,MultiplayerEnumerations::TYPE tempType):CURRENTWALKINGSTAT(MultiplayerEnumerations::PSY_DOWN){this->p_s_Position = new MultiplayerStructs::Vector2D;p_s_Velocity = new MultiplayerStructs::Vector2D;p_s_Position->X = x;p_s_Position->Y = y;CURRENTTYPE = tempType;this->p_AnimationTimer = new MultiplayerHighClass::Timer(); this->p_AnimationTimer->start();}
